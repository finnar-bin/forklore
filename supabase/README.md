...
1. Push `migrations/20260828000000_ingredients_recipes_updated_by.sql` (included in `supabase:push:dev`/`supabase:push:prod`) — adds `ingredients.updated_by`/`recipes.updated_by` and updates the `recalculate_recipe_kcal` trigger to set `updated_by`.
...
13. Record the result somewhere durable (PR description, ticket comment) — same as prior tickets, this is the acceptance criterion, not something provable from the code alone.
