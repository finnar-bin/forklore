...
6. Group route guard: as a user who is a member of at least one group, note that group's id, then log out and log back in as a different user who is _not_ a member of it → manually navigate to `/groups/<that id>/pantry` → confirm you're redirected to `/groups` instead of seeing that group's (possibly cached) pantry.
...
1. `npm run dev`, log in as a member of at least one group. On `/pantry`, open an ingredient, tap the `MoreVert` icon top-right of the card, and confirm the menu shows "Copy" alongside "Delete" (see "Ingredient/recipe detail actions menu" in `docs/pending-deviations.md` — these were standalone buttons at the time this ticket was written, since consolidated into a menu).
...
3. Group → personal and group → group: repeat from a group's pantry screen, confirming "Personal" is offered and, if you're in 2+ groups, every _other_ group is offered (not the one currently being viewed).
4. On `/recipes`, open a recipe with at least one ingredient, tap the `MoreVert` icon next to the photo, and confirm the same menu shows "Copy" next to "Delete", and that "Copy" is disabled while there are unsaved edits on the screen (change the name without saving, confirm it's disabled; save or revert, confirm it re-enables).
...
11. `use_existing_id` tamper check: call `supabase.rpc('copy_recipe', { p_recipe_id: '<a recipe you own, with at least one ingredient>', p_target_group_id: null, p_ingredient_resolutions: [{ source_ingredient_id: '<that ingredient's id>', use_existing_id: '<some other ingredient id that is NOT a genuine name+unit match in your personal context>' }] })` directly → confirm the recipe is still created (doesn't raise), but the ingredient in question got a _fresh_ copy rather than being linked to the id you supplied — i.e. the server silently ignored the bad `use_existing_id` rather than trusting it.
12. Record the result somewhere durable (PR description, ticket comment) — same as prior tickets.

## Community pantry manual verification

Needs `supabase/migrations/20260907000000_community_pantry.sql` pushed — adds `ingredients.is_community`, `profiles.community_pantry_enabled`, `groups.community_pantry_enabled`, widens the ingredients select RLS policy, adds `check_community_ingredient_usage`, and re-declares `copy_ingredient`/`copy_recipe` with a widened source-access check (see `docs/pending-deviations.md`, "Community pantry").

1. `npm run dev`, log in as user A. Visit `/pantry` → tap "Browse community pantry" → confirm it navigates to `/community-pantry`. Tap its FAB, create an ingredient → confirm it appears in the list with "Added by A" (no border/tab here — `CommunityPantryList.tsx` suppresses the indicator since every row on this page is already known to be a community ingredient).
2. Log in as user B (no relationship to A — not a group-mate). Visit `/community-pantry` → confirm A's ingredient is visible (community ingredients are readable by everyone regardless of any opt-in switch). Open it → confirm it's read-only (no editable fields, no actions-menu "Delete" item) and shows "Added by A"; confirm the actions menu's "Copy" item is still available and, used, creates an independent, fully-editable copy in B's own personal pantry (Table Editor: new `ingredients` row, `created_by = B`, `is_community = false`).
3. As B, visit `/pantry` → confirm A's community ingredient does **not** appear (B's `community_pantry_enabled` is still off), and confirm a "Use community pantry ingredients" switch appears directly on this screen (not on `/profile`). Turn it on → confirm it takes effect immediately, with no separate save step: A's community ingredient now appears, with a `secondary.main`-colored card border and a top-left "Community" tab (mixed-context lists like this one show the indicator; `/community-pantry`'s own list, from step 1, does not).
4. As B (now opted in), open "Log an entry" from `/log` → confirm A's community ingredient is selectable, labeled "Community" (not "Personal"); log it → confirm the resulting entry snapshots correctly and is unaffected by anything happening to the source ingredient afterward. Open a recipe's "Add ingredient" → "From pantry" → confirm it's selectable there, and that once added, the recipe's own ingredient list (`RecipeIngredientsList.tsx`) shows the same border+tab indicator on that row.
5. As A, open the community ingredient from `/community-pantry` → confirm it's fully editable (A is the creator) and the actions menu's "Delete" item is present. Add it to a recipe as A first (so usage exists across two different users' recipes — one of A's, one of B's from step 4/2), then attempt to delete it → confirm the dialog shows the recipe(s) A can see by name (existing `check_ingredient_usage` behavior) **and** a separate total count that includes B's recipe even though A can't see B's recipe by name (`check_community_ingredient_usage`).
6. Group-level: as the owner of a group, visit `/groups/:id/pantry` → confirm a "Use community pantry ingredients in this group" switch appears there (not on `/groups/:id/settings`), turn it on → confirm it takes effect immediately. As a member of that group who is _not_ the owner, visit the same screen → confirm the switch itself isn't shown (owner-only), but confirm community ingredients now appear and are usable in that group's recipes/log. Confirm a _different_ group (switch still off) shows neither the switch's effect nor (if you're not its owner) the switch.
...
9. Record the result somewhere durable (PR description, ticket comment) — same as prior tickets.
...
## log_entries live-reference rework manual verification

Needs `supabase/migrations/20260909000000_log_entries_rework.sql` pushed — drops and recreates `log_entries` (column rename off the old `snapshot_*` naming, `quantity`/`unit` now `not null`) and its indexes/RLS policies (see `docs/pending-deviations.md`, "log_entries reworked from snapshot to live-reference"). **This drops the existing `log_entries` table** — confirm there are no rows worth keeping in whichever project this is pushed to before running it (there shouldn't be any yet).

1. `npm run dev`, log in. Log an ingredient and a recipe from `/log`'s FAB (or `/groups/:groupId/log`'s) → confirm both appear with correct kcal, same as before this change.
2. Tap a logged ingredient entry to open `EditLogEntryDialog` → confirm it shows the same live kcal-preview header as the "Log an entry" step (name + its own quantity/unit, group label, kcal + rate), a single editable "Quantity" field prefilled with the logged amount, and the meal-type selector — no separate Name/Kcal text fields anymore.
3. While that entry's source ingredient still exists, go correct its kcal value on `/pantry` (or `/groups/:groupId/pantry`), then reopen the same log entry → confirm the header's kcal figure already reflects the _new_ rate before you touch the quantity field (proves it's reading the live source, not a frozen value). Change the quantity and tap "Save changes" → confirm the entry's kcal in `/log`/`/logs` updates to match the new rate × the new quantity, and the running daily total recomputes.
4. Repeat steps 2–3 for a recipe-sourced entry (`RecipeKcalHeader`, grams-eaten field) — including typing an amount above the recipe's `weight_g` and confirming the same "Can't exceed this recipe's total weight" validation the creation step has.
5. Delete the source ingredient (or recipe) of an already-logged entry from `/pantry`/`/recipes`, then reopen that log entry → confirm it renders the same header + quantity layout as normal, but fully locked: the quantity field, meal-type selector, and "Save changes" button are all disabled (no editable fields left at all), a bold red "Source deleted — this entry is locked at its last known values." line appears at the bottom of the dialog body, and the entry's kcal in `/log`/`/logs` is unchanged from before the source was deleted. Confirm "Delete log entry" still works from this locked state.
6. Confirm deleting a log entry still works from both the normal and the source-deleted/locked state.
7. Confirm `/groups/:groupId/log`'s per-member kcal breakdown (`GroupMemberKcalCard`) and `/logs`' daily totals still sum correctly after editing/refreshing an entry's kcal via step 3.
8. Direct RLS spot-check (defense in depth — this migration restates access control, doesn't change it, but worth confirming after a drop+recreate): as a group member who isn't the entry's own logger, attempt a direct `update`/`delete` against the `log_entries` REST endpoint for another member's entry in a shared group → confirm it's rejected ("update own log entries" / "delete own log entries" are owner-only), while a plain `select` on that same row still succeeds (group-inclusive read).
9. Weight-shrink regression check (fix, no DB change): log a recipe entry, then reduce that recipe's `weight_g` below the logged quantity on its detail page, then reopen the log entry → confirm the quantity field shows red/over-max but "Save changes" stays enabled for a meal-type-only change; confirm actually editing the quantity re-enables the same over-max validation.
10. Unit-mismatch lock check (fix, no DB change): log an ingredient entry, then change that ingredient's unit on its detail page (e.g. g → piece), then reopen the log entry → confirm the whole form locks (quantity/meal-type/Save all disabled) with a "This ingredient's unit has changed since logging…" message, and that the shown kcal/quantity are the entry's last-saved values, not a live recomputation against the new unit.
11. Record the result somewhere durable (PR description, ticket comment) — same as prior entries.

## log_entries "log for a group member" manual verification

Needs `supabase/migrations/20260910000000_log_entries_logged_for.sql` pushed — renames `log_entries.logged_by` to `logged_for`, adds a new `created_by` column (backfilled from `logged_for`), and replaces the insert/update/delete RLS policies (see `docs/pending-deviations.md`, "log_entries: log an entry on a fellow group member's behalf"). Not a drop+recreate — existing rows survive with `created_by` backfilled equal to their own `logged_for`.

1. Create (or use) a group with at least two members — call them A and B. Log in as A, open a group's "Log an entry" from `/groups/:groupId/log`, pick a group-owned ingredient or recipe → confirm a "Log for" field appears below the meal-type selector, defaulting to "A" (yourself), listing every member of that group by name.
2. Still as A, pick "B" in the "Log for" field and log the entry → confirm it lands on the group's shared log, and its card shows "For B · logged by A" (A viewing their own creation, logged for someone else).
3. Log out, log in as B → visit that same group's log and confirm the same entry appears; visit B's own personal `/log` (cross-context "today" view) and confirm it appears there too, counted toward B's own daily total, even though B didn't create it.
4. As B, open that entry (tap it) → confirm `EditLogEntryDialog` opens (group-inclusive edit — B isn't the creator but the update RLS now allows it), shows the "For B · logged by A" attribution line under the header, and that quantity/meal-type/Save all work normally; change the quantity and save → confirm it persists.
5. As A, reopen the same entry → confirm A can also edit/delete it (group-inclusive, not creator-only).
6. Pick a _personal_ (not group-owned) ingredient/recipe from `/log`'s FAB (personal context, no `groupId`) → confirm no "Log for" field appears at all, and the resulting entry is attributed to yourself regardless.
7. In a group with only one member (just yourself) — or right after creating a brand-new group — confirm the "Log for" field doesn't render (`LoggedForSelector` hides itself when there's nobody else to pick).
8. Direct RLS spot-check: as a user with no membership in a group, attempt a direct `insert` against the `log_entries` REST endpoint with that group's `id` as `group_id` → confirm it's rejected; as a member of that group, attempt an `insert` with `logged_for` set to a user _not_ in that group → confirm it's rejected too (the insert policy's second `group_id in (...)` clause, keyed on `logged_for` rather than the caller).
9. Member-removal edge case (documented limitation, not fixed): log an entry for member B, then remove B from the group (`/groups/:groupId/settings`) → confirm the entry still appears in the group's history with B's name still resolving correctly, but attempting to edit/delete it as anyone now surfaces a permission-denied message on `/sync-status` (`logged_for` no longer belongs to a current member, and B themselves is no longer a member either) rather than silently succeeding.
10. Community ingredients follow-up (no DB change): as a member of a 2+-person group with community pantry enabled, open "Log an entry" from that group's own `/groups/:groupId/log` screen (not the personal `/log` screen), pick a community ingredient → confirm the "Log for" field now appears; pick a different member and log it → confirm the entry lands on that group's own shared log (not your personal log) with the usual "For X · logged by Y" attribution. Then open `/log` (personal, cross-context) and pick the same community ingredient → confirm no "Log for" field appears there and it logs to yourself.
11. Meal-type selector follows the logged-for target's own setting (no DB change): with your own `meal_breakdown_enabled` off, log a personal item → confirm no meal-type selector appears; turn it on (Profile settings) and log again → confirm it appears. In a 2+-member group where you have it on and a fellow member doesn't, open "Log an entry" from that group's log, pick a group item, switch "Log for" between yourself and that member → confirm the meal-type selector toggles live as the picker changes. Open an entry logged for a member without breakdown enabled via `EditLogEntryDialog` → confirm the selector is hidden there too.
12. Record the result somewhere durable (PR description, ticket comment) — same as prior entries.

## Remove personal mode manual verification

Needs both `supabase/migrations/20260911000000_backfill_personal_to_default_groups.sql` and `supabase/migrations/20260912000000_require_group_membership.sql` pushed, **in that order** — the second adds `not null`/check constraints that any remaining personal row would violate (see `docs/pending-deviations.md`, "Remove personal mode — require every account to belong to a group"). Run the backfill, do steps 1–3 below, only then push the constraint migration and do the rest.

**Also needs a dashboard config check, not just the migrations** — the invite-before-onboarding handoff (steps 7–8) makes `signUpWithEmail`/`signInWithGoogle` pass a dynamic `emailRedirectTo`/`redirectTo` of `<origin>/invite/<code>` instead of the previous fixed `<origin>`. Under Authentication → URL Configuration, confirm the project's "Redirect URLs" allow-list includes a pattern that matches this (e.g. `<your-site-url>/**`, not just the bare origin) — if it only allow-lists the exact origin, Supabase silently ignores the requested redirect and sends the user to the default Site URL instead, which would make steps 7–8 look like the handoff isn't working when the actual cause is this dashboard setting, not the code.

1. **Before pushing the constraint migration:** in the Table Editor, confirm no row remains with `group_id is null` in `recipes` or `log_entries`, and none with `group_id is null and is_community = false` in `ingredients`. If any remain, the backfill's user-selection query missed them — investigate before proceeding rather than pushing the constraint migration anyway.
2. For a handful of users who previously had personal data and no group, confirm a new group now exists (named `"<their name>'s Kitchen"`), they're its `owner` in `group_members`, and their previously-personal rows now carry that group's id with a fresh `updated_at`.
3. For a user who already belonged to a group and also had personal rows, confirm those rows were reassigned to their earliest-joined group (`group_members.joined_at`), not a newly-created one.
4. **After pushing the constraint migration:** attempt a direct `insert` against the `recipes` or `log_entries` REST endpoint with `group_id` omitted/null → confirm it's rejected (`not null` violation). Attempt the same against `ingredients` with `is_community = false` and `group_id` omitted → confirm the new `ingredients_group_or_community` check rejects it; confirm an `is_community = true` insert with `group_id` omitted still succeeds.
5. `supabase.rpc('copy_ingredient', { p_ingredient_id: '<any ingredient you can see>', p_target_group_id: null })` (and `copy_recipe`/`find_ingredient_match` the same way) directly → confirm each now rejects/returns nothing rather than treating `null` as "personal."
6. `npm run dev`, sign up a brand-new account (email/password) with no invite involved → confirm onboarding's final step now requires creating or joining a group before `onboardingComplete` becomes true, and that finishing it lands on `/groups` (not `/groups/:groupId/pantry` directly, and not `/pantry`, which should no longer be reachable — confirm the bare URL redirects or 404s gracefully rather than crashing) showing the group just created/joined.
7. Invite-before-onboarding handoff: as an existing user who owns a group, generate an invite link. Open it in a fresh/incognito session (logged out) → confirm the invite preview renders with "Sign up to join" / "Log in" instead of "Join group." Tap "Sign up," complete signup, confirm the email, and confirm the confirmation link lands back on the same invite page, now authenticated, and that accepting it there works. Proceed into onboarding afterward and confirm the group step recognizes `hasGroup` as already satisfied rather than prompting to create/join again.
8. Repeat step 7 via "Log in" instead, using an existing account that has no groups yet — confirm login navigates straight to the invite page and the same handoff works.
9. A returning user who completed the profile steps in an earlier session but never finished the group step (simulate by stopping onboarding right before the last step) → confirm reopening `/onboarding` jumps straight to the group step rather than restarting from "About you."
10. `GroupCard` click-to-switch: on `/groups`, tap a card's body (not the invite/settings icons) → confirm it navigates straight into that group's pantry; reload the app → confirm it now lands there directly (the pick persisted). As an owner, tap the invite icon and the settings gear → confirm each opens its own dialog/screen without also navigating into the card.
11. Land-on-`/groups`-when-nothing-picked: clear `localStorage` (or use a fresh browser profile) and log in as a user who belongs to 2+ groups → confirm landing on `/groups`, not a silently-guessed one. From `/progress` (also nothing stored), tap the Pantry or Recipes bottom-nav tab → confirm it also goes to `/groups` rather than guessing. Then tap a group card and confirm subsequent logins/reloads go straight to it.
12. Record the result somewhere durable (PR description, ticket comment) — same as prior entries.
