...
6. Pick a *personal* (not group-owned) ingredient/recipe from `/log`'s FAB (personal context, no `groupId`) → confirm no "Log for" field appears at all, and the resulting entry is attributed to yourself regardless.
...
8. Direct RLS spot-check: as a user with no membership in a group, attempt a direct `insert` against the `log_entries` REST endpoint with that group's `id` as `group_id` → confirm it's rejected; as a member of that group, attempt an `insert` with `logged_for` set to a user *not* in that group → confirm it's rejected too (the insert policy's second `group_id in (...)` clause, keyed on `logged_for` rather than the caller).
