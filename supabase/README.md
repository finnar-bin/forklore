...
6. `npm run dev`, sign up a brand-new account (email/password) with no invite involved → confirm onboarding's final step now requires creating or joining a group before `onboardingComplete` becomes true, and that finishing it lands on `/groups/:groupId/pantry`, not `/pantry` (which should no longer be reachable — confirm the bare URL redirects or 404s gracefully rather than crashing).
...
10. Record the result somewhere durable (PR description, ticket comment) — same as prior entries.
