...
20. "View group logs": on `/log`, confirm a "View group logs" button appears beside "View all-time history" → tap it, confirm `/logs/groups` lists every group A belongs to, and tapping one navigates to `/groups/<id>/log` for that group.
...
22. Record the result somewhere durable (PR description, ticket comment) — same as prior tickets, this is the acceptance criterion, not something provable from the code alone.
