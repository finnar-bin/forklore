...
1. Push `migrations/20260828000000_ingredients_recipes_updated_by.sql` (adds `ingredients.updated_by`/`recipes.updated_by`, updates the `recalculate_recipe_kcal` trigger to set `updated_by`) and `migrations/20260829000000_recipes_servings_to_weight.sql` (renames `recipes.servings` to `weight_g`, retyped to `numeric`) — both included in `supabase:push:dev`/`supabase:push:prod`.
...
16. Record the result somewhere durable (PR description, ticket comment) — same as prior tickets, this is the acceptance criterion, not something provable from the code alone.
