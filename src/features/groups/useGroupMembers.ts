...
const useGroupMembersCacheStore = create<CacheState & CacheActions>((set, get) => ({
  groupId: null,
  members: undefined,
  inflight: null,
  ...
  ensureLoaded: (groupId) => {
    const state = get();
    if (state.groupId === groupId && (state.members !== undefined || state.inflight)) return;
...
export function useGroupMembers(groupId: string | null): GroupMember[] | undefined {
  const members = useGroupMembersCacheStore((state) => (state.groupId === groupId ? state.members : undefined));
  const inflight = useGroupMembersCacheStore((state) => (state.groupId === groupId ? state.inflight : null));
