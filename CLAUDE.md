...
npm run test:e2e            # playwright test (e2e/) — see e2e/README.md
npm run test:e2e:ui         # playwright test --ui
```

There is no unit/component test framework configured in this repo (no jest/vitest) — do not assume one exists. There is an e2e suite (`e2e/`, Playwright) — see `e2e/README.md` before adding to it, especially for why/how it mocks Supabase entirely rather than connecting to a real project (required by the "must not connect to databases" policy below). `tsc -b` (via the build scripts), `oxlint`, and `prettier --check` remain the baseline automated verification for any change; treat a clean run of all three (plus `npm run test:e2e` when you've touched a flow it covers) as the bar before considering a change done.
...
- **Keep `e2e/` in sync with behavior changes.** If a change adds, removes, or alters a user-facing flow in an area `e2e/specs/` already covers (auth, onboarding, pantry/recipe/log CRUD, groups/invites, community pantry, offline sync), update the corresponding spec in the same change — a new field, button, or redirect that a spec asserts on will otherwise silently start failing (or worse, silently stop testing anything real) the next time someone runs `npm run test:e2e`. A genuinely new flow in one of those areas should get a new spec, not be left uncovered. If a change touches `src/lib/db.ts`'s Dexie schema, an RPC's params/shape, or adds a new Supabase table/RPC/Edge Function the app calls, check whether `e2e/mocks/mockBackend.ts` needs the matching update too (a new RPC case, a new table in `TABLES`, an `EMBEDS` entry for a new embedded-select query) — otherwise any spec that exercises it will hit the mock's 404 fallback instead of real behavior. Run `npm run test:e2e` before considering such a change done, per the Commands section above. Areas `e2e/README.md` lists as not yet covered (photo upload, progress/weight charts, the converter) don't need a spec added on their account alone, but don't let that grow into an excuse to skip coverage for a flow that clearly belongs in one of the already-covered areas.
...
- Keep code comments succinct — one or two lines stating the non-obvious "why," not a multi-line wall of text. If a comment needs more than that to explain itself, that's a sign it belongs in `docs/pending-deviations.md` or the PR description instead.
